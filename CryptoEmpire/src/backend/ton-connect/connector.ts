...
export function getConnector(initData: string): { connector: TonConnect, userId: number | string } | null {
    ...
    if (!connector) {
        if (!process.env.MANIFEST_URL) {
             console.error("MANIFEST_URL is not set in .env. This is required to create a new TonConnect connector.");
             throw new Error("MANIFEST_URL is not set in environment variables.");
        }
